...
public function addmanufacturer($manufacturername) {
    ...
    if (strpos('x'.strtolower($manufacturername),'monkey') > 0) $manufacturername = 679;
    ...
    if ($search)
        ...
        $manu->active=1;
        if ((Validate::isCatalogName($manufacturername)) and ($manufacturername != '')) {
        if ($manu->add()) $idmanufacturer=$manu->id_manufacturer; 
        }
        else $idmanufacturer = 0;
    }
...
if ($added_img > 0) $cover=0; 
else $cover=1;         
    
$url=$im;    
if (strpos('x'.$url,'http') > 0) $donothing=1;
else $url = 'http://'.$url;    
    $image = new Image();
...
public function removediscounts($idproduct,$discountpercentage,$dodiscount = false) {
...
public function adddiscount($idproduct,$discountpercentage,$dodiscount = false) {
...
public function setweightfromprice($pret) {
    
    $weight=0;
    
    if ($pret <= 50) $weight=0.5; 
    else if ($pret <= 100) $weight=1; 
    else if ($pret <= 250) $weight=3; 
    else if ($pret <= 300) $weight=7; 
    else if ($pret <= 500) $weight=10; 
    else if ($pret <= 1000) $weight=20; 
    else if ($pret <= 1800) $weight=25; 
    else if ($pret > 1800) $weight=35; 
...
if (isset($feedprod['ean13'])) {
    $prod->ean13 = pSQL($feedprod['ean13']);
}
    
if (isset($feedprod['on_sale'])) {
    $prod->on_sale = pSQL($feedprod['on_sale']);
}
...
if (isset($feedprod['wholesale_price']))
    $prod->wholesale_price = $feedprod['wholesale_price'];
...
$feedprod['description'] = str_replace("\n", "<br />", $feedprod['description']);
    
    
if (strlen($feedprod['description']) > 390) {   
$propozitii=explode('.',strip_tags($feedprod['description']));
...
if ($descrierescurta == '') $descrierescurta=substr(strip_tags($feedprod['description']),0,200);
...
$prod->weight = $this->setweightfromprice($prod->price);   
...
if ($prod->add()) { 
    ...
    if (isset($feedprod['dodiscount'])) {
    if ($feedprod['discount'] > 0) $this->adddiscount($prod->id,$feedprod['discount'],true);
    }
    ...
    $getwarehouse=Db::getInstance()->getRow("SELECT id_warehouse FROM "._DB_PREFIX_."warehouse WHERE reference='".pSQL($feedprod['numefeed'])."'");
    if ($getwarehouse) {
    Warehouse::setProductLocation($prod->id,$id_product_attribute,$getwarehouse['id_warehouse'],'');

    $addstoc=Db::getInstance()->Execute("INSERT INTO "._DB_PREFIX_."stock (id_warehouse, id_product, id_product_attribute, reference, physical_quantity, usable_quantity) VALUES ('".pSQL($getwarehouse['id_warehouse'])."', '".pSQL($prod->id)."', '".$id_product_attribute."', '".pSQL($feedprod['reference'])."', '".pSQL($feedprod['stock'])."', '".pSQL($feedprod['stock'])."')"); 
    }
...
$defattribute=0;
$getcurrentquantity=Db::getInstance()->getRow("SELECT quantity FROM "._DB_PREFIX_."stock_available WHERE id_product='$id_product' AND id_product_attribute='$defattribute'"); 
    
if ($getcurrentquantity['quantity'] < 600) {    
...
$id_product_attribute=0;   
    ...
    $updatestocinware=Db::getInstance()->Execute("UPDATE "._DB_PREFIX_."stock SET physical_quantity='".pSQL($feedprod['stock'])."', usable_quantity='".pSQL($feedprod['stock'])."' WHERE id_warehouse = '".pSQL($getwarehouse['id_warehouse'])."' AND id_product='".pSQL($id_product)."'"); 
    }    
...
public function updatestockcomb($feedprod, $id_product, $id_product_attribute) {
    
    
    $update_stock = Db::getInstance()->Execute("UPDATE "._DB_PREFIX_."stock_available SET quantity='".(int)$feedprod['stock']."' WHERE id_product='".pSQL($id_product)."' AND id_product_attribute='".pSQL($id_product_attribute)."' AND id_shop=1");
    
    if ($update_stock) {
            if ($id_product_attribute > 0) {
                $canttotala = Db::getInstance()->getValue("SELECT SUM(quantity) FROM "._DB_PREFIX_."stock_available WHERE id_product='".pSQL($id_product)."' AND id_product_attribute>'0' AND id_shop=1");

                if ($canttotala !== null) {
                $res = Db::getInstance()->Execute("UPDATE "._DB_PREFIX_."stock_available SET quantity='".pSQL($canttotala)."' WHERE id_product='".pSQL($id_product)."' AND id_product_attribute='0' AND id_shop=1");            
                }
            }     
    }
...
public function checkprice($feedprod,$taxrule,$tva,$id_product) {
    ...
    if (isset($feedprod['ignore_pricealert']))
        return true;
...
if ($feedprod['discount'] <= 0)     
$feedprod['discount']=0;       
    
if (isset($feedprod['dodiscount'])) {    
if ($feedprod['discount'] > 0) $this->adddiscount($id_product,$feedprod['discount'],true);
else $this->removediscounts($id_product,$feedprod['discount'],true);
}
