...
final class SupplierFormDataHandler implements FormDataHandlerInterface
{
    ...
    public function create(array $data)
    {
        ...
        //add fake languages
        Db::getInstance()->Execute("INSERT INTO "._DB_PREFIX_."supplier_lang (id_supplier, id_lang) VALUES ('".$supplierId->getValue()."', '1')");
        Db::getInstance()->Execute("INSERT INTO "._DB_PREFIX_."supplier_lang (id_supplier, id_lang) VALUES ('".$supplierId->getValue()."', '2')");
        
        Db::getInstance()->Execute("UPDATE "._DB_PREFIX_."supplier SET feed = '".$data['feed']."', adaos = '".$data['adaos']."' WHERE id_supplier='".$supplierId->getValue()."'");
    ...
    private function fillCommandWithData(EditSupplierCommand $command, array $data)
    {
        ...
        if (null !== $data['feed']) {
            $command->setFeed($data['feed']);
        }
        
        if (null !== $data['adaos']) {
            $command->setAdaos($data['adaos']);
        }
