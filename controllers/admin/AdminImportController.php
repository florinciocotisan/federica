...
class AdminImportControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        //$this->csv_file = 'feed_'.Tools::getValue('id_supplier').'.csv';
        
        if (Tools::getValue('id_supplier')) 
            Configuration::updateValue("CURRENT_SUP_MATCH", Tools::getValue('id_supplier'));
        
        $id_supplier = (int)Configuration::get("CURRENT_SUP_MATCH");
        
        $this->csv_file = 'feed_'.$id_supplier.'.csv';
        
        
        //var_dump($this->entities[$this->trans('Products', [], 'Admin.Global')]);
        
        
        switch ((int) $this->forced_entity) {
    ...
    public function renderView()
    {
        ...
        if (Tools::getValue('id_supplier')) {
        $this->context->cookie->entity_selected = 1;
        $this->context->cookie->iso_lang_selected = 'en';
        ...
        $this->context->cookie->csv_selected = urlencode($this->csv_file);    
        }
        else {   
        $this->context->cookie->entity_selected = (int) $this->forced_entity;
        $this->context->cookie->iso_lang_selected = urlencode($this->iso_lang);
        $this->context->cookie->separator_selected = urlencode($this->separator);
        $this->context->cookie->multiple_value_separator_selected = urlencode($this->multiple_value_separator);
        $this->context->cookie->csv_selected = urlencode($this->csv_file);
        }
    ...
    {
        //&csv=1&id_supplier=7 add this to url
        //$this->content .= 'test'.Tools::getValue('id_supplier');
        
        if (Tools::getValue('id_supplier')) {
            $this->content .= $this->renderView();
        }
        else if ($this->display == 'import') {
            if ($this->csv_file) {
                $this->content .= $this->renderView();
            } else {
    ...
    protected static function fillInfo($infos, $key, &$entity)
    {
        ...
        //PrestaShopLogger::addLog(json_encode($infos), 1);
        $infos = trim($infos);
        
        if (isset(self::$validators[$key][1]) && self::$validators[$key][1] == 'createMultiLangField' && $iso_lang) {
    ...
    public static function arrayWalk(&$array, $funcname, &$user_data = false)
    {
        ...
        foreach ($array as $k => $row) {
            ...
            //PrestaShopLogger::addLog(json_encode($funcname).' '.$row.' '.$key.' '.json_encode($user_data), 1);
            
            if (!call_user_func_array($funcname, [$row, $k, &$user_data])) {
                return false;
            }
            //die(json_encode('test19'));
        }
    ...
    public function productImport($offset = false, $limit = false, &$crossStepsVariables = false, $validateOnly = false, $moreStep = 0)
    {
        ...
        $parts = explode('_',str_replace('.csv', '', $offset));
        $id_supplier = (int)$parts[1];                 
    ...
    protected function productImportOne($info, $default_language_id, $id_lang, $force_ids, $regenerate, $shop_is_feature_active, $shop_ids, $match_ref, &$accessories, $validateOnly = false)
    {
        ...
        $supplier_data = Db::getInstance()->getRow("SELECT * FROM "._DB_PREFIX_."supplier WHERE id_supplier='".pSQL($id_supplier)."'");
        
        $adaos = 1 + ($supplier_data['adaos'] / 100);
        ...
        $product->price = $product->price * $adaos;
        
        $id_supplier_cat = Db::getInstance()->getValue("SELECT id_category FROM "._DB_PREFIX_."category_lang WHERE LOWER(name)='".pSQL(strtolower($supplier_data['name']))."'");
        
       
        if (!empty($product->category) && is_array($product->category)) {
            $cats = implode(' - ',$product->category);
            $product->category = array();
            $product->category[] = $cats;
            $product->category[] = $id_supplier_cat;
            ...
            foreach ($product->category as $value) {
                if (is_numeric($value)) {
                    if (Category::categoryExists((int) $value)) {
                        ...
                        $category_to_create->active = true;
                        $category_to_create->id_parent = $id_supplier_cat;
                    ...
                    $category = Category::searchByPath($default_language_id, trim($value), $this, 'productImportCreateCat', $id_supplier_cat);
    ...
    protected function attributeImportOne($info, $default_language, &$groups, &$attributes, $regenerate, $shop_is_feature_active, $validateOnly = false)
    {
        ...
        } elseif ($this->match_ref && isset($info['product_reference']) && $info['product_reference']) {
            $datas = Db::getInstance()->getRow('
				SELECT p.`id_product`
				FROM `' . _DB_PREFIX_ . 'product` p
				' . Shop::addSqlAssociation('product', 'p') . '
    ...
    {
        if (Tools::getValue('id_supplier'))
        $file = $this->convertFeedtoCSV(Tools::getValue('id_supplier'));
        else 
        $file = $this->convertFeedtoCSV((int)Configuration::get("CURRENT_SUP_MATCH"));
    ...
    public function convertFeedtoCSV($id_supplier) {
        
        $url_feed = Db::getInstance()->getValue("SELECT feed FROM "._DB_PREFIX_."supplier WHERE id_supplier='".pSQL($id_supplier)."'");
        
        if (!$url_feed) return false;
        $path_to_feed = $this->downloadSupplierFeed($url_feed, $id_supplier);
        
        
        
        $reader = new XMLReader();
        $reader->open($path_to_feed);

        $elementCounter = 0;
        
        //die('das');
        //establish the repeating items
        while ($reader->read()) {
            if ($reader->nodeType == XMLReader::ELEMENT) {
                ...
                $endoffile = false;
                $iterations = 0;
                while ($nonodes < 2 && $endoffile == false && $iterations < 10) {
                    $iterations++;
                    $internaiterations=0;
                    foreach ($xml as $xmlkey=>$xmlval) {
                        $internaiterations++;
                        if (count($xml->$xmlkey) > 4) { $productskey = $xmlkey; $productsnode = $xml; }
                        //echo $xmlkey.' - '.$xmlval.'<br />';
                        if ($internaiterations > 10) break;
                    }
                    
                    if (!isset($xml->$xmlkey)) { $endoffile = true; break; }
                    $xml = $xml->$xmlkey;
        ...
        if (!isset($productskey)) return false;
        //echo $productskey.'- ';
        //var_dump($productsnode);
        ...
        $ajunslaproducts = false;
        
        $csvfiedls = array();
        $csvfiedheader = array();
        $csvfiedfirstrow = array();
        ...
        while ($reader->read()) {
            ...
            if ($reader->nodeType == XMLReader::ELEMENT && $reader->localName == $productskey) {
                $ajunslaproducts = true;
            } 
            
            if ($ajunslaproducts && $reader->localName != $productskey) {
                if ($reader->localName) {
                    if (strpos('x'.$reader->localName,'#') === false) {
                        $csvfiedheader[strval($reader->localName)] = $reader->localName;
                        $lastlocaname = $reader->localName;
                    }
                        
                }
                
                if (trim($reader->value) != '') {
                    if (isset($csvfiedheader[strval($lastlocaname)])) {
                    $csvfiedfirstrow[strval($lastlocaname)][] = $reader->value;
                    }
                }
        ...
        foreach ($csvfiedheader as $hv)
            $csvfiedls[0][] = $hv;
        
        $i=0;
        $aux = $csvfiedfirstrow;
        //echo count($csvfiedfirstrow);
        foreach ($csvfiedfirstrow as $hkey=>$hv) {  
            foreach ($aux as $hkey2=>$hv2) {
                if (isset($hv2[$i]))
                    $csvfiedls[$i+1][] = $hv2[$i];
            }
            $i++;
        ...
        $pathtocsv = _PS_ROOT_DIR_.'/administration/import/feed_'.$id_supplier.'.csv';
        
        $fp = fopen($pathtocsv,'w');
            
        $buffer = '';

        foreach ($csvfiedls as $fields)
            {
             fputcsv($fp, $fields, ";");
             }
    ...
    public function downloadSupplierFeed($url_feed, $id_supplier) {
        
        if (!file_exists(_PS_ROOT_DIR_.'/import/feeds/'.$id_supplier)) {
            mkdir(_PS_ROOT_DIR_.'/import/feeds/'.$id_supplier, 0755, true);
        }
        
        //add category
        
        $supplier_name = Db::getInstance()->getValue("SELECT name FROM "._DB_PREFIX_."supplier WHERE id_supplier='".pSQL($id_supplier)."'");
        
        $search=Db::getInstance()->getRow("SELECT id_category FROM "._DB_PREFIX_."category_lang WHERE name='$supplier_name'");
        ...
        if (!$search) {
            ...
            $catob->name[1]=$supplier_name;
            $catob->name[2]=$supplier_name;

            $catob->link_rewrite[1]=Tools::link_rewrite($supplier_name,true);
            $catob->link_rewrite[2]=Tools::link_rewrite($supplier_name,true);    
            $res = $catob->add();
            
        }
        
        $path_to_feed = _PS_ROOT_DIR_.'/import/feeds/'.$id_supplier.'/feed.xml';
        if (time() - filemtime($path_to_feed) > 23*3600 ) {
            $url = $url_feed;
            $fp = fopen($path_to_feed, 'wb'); 
    ...
    public function importByGroups($offset = false, $limit = false, &$results = null, $validateOnly = false, $moreStep = 0)
    {
        ...
        if ($this->csv_file) {
            ...
            if (!$offset && !$moreStep && !$validateOnly && (($shop_is_feature_active && $this->context->employee->isSuperAdmin()) || !$shop_is_feature_active) && $this->truncate) {
                $this->truncateTables((int) $this->forced_entity);
            }
            ...
            switch ((int) $this->forced_entity) {
                ...
                case $this->entities[$import_type = $this->trans('Products', [], 'Admin.Global')]:
                    ...
                    //die(json_encode('test6'.$this->forced_entity));
                    $clearCache = true;
            ...
            //die(json_encode('test4'.$this->forced_entity));
            // @since 1.5.0
            if (Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
                switch ((int) $this->forced_entity) {
    ...
    public function ajaxProcessImport()
    {
        ...
        if (!$validateOnly && (bool) $results['isFinished'] && !isset($results['oneMoreStep']) && (bool) $this->sendemail) {
