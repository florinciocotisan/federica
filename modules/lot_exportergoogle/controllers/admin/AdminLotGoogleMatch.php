...
class AdminLotGoogleMatchController extends ModuleAdminController
{
	...
	public function __construct()
	{
        ...
        $this->gcats = Db::getInstance()->ExecuteS("SELECT cl.id_category, cl.name FROM "._DB_PREFIX_."category_lang cl INNER JOIN "._DB_PREFIX_."category c ON cl.id_category=c.id_category WHERE c.id_parent=3 AND id_lang=2");
    ...
    public function getGoogleCategory($content, $tr) {
        ...
        //$getGoogleCat = Db::getInstance()->getRow("SELECT cl.id_category, cl.name FROM "._DB_PREFIX_."category_lang cl INNER JOIN "._DB_PREFIX_."category c ON cl.id_category=c.id_category WHERE c.id_parent=3 AND id_lang=2 AND cl.id_category='".pSQL($content)."'");
        
        $getGoogleCat = Db::getInstance()->getRow("SELECT gm.google_id, cl.name FROM "._DB_PREFIX_."lot_exportergoogle_category_map gm LEFT JOIN "._DB_PREFIX_."category_lang cl ON cl.id_category=gm.google_id WHERE gm.id_category='".pSQL($content)."' AND cl.id_lang=2");
        
        if ($getGoogleCat) return '<input type="text" id="gcatselect_'.$content.'" class="gcatselect" data-id_category="'.$content.'" data-google_id="'.$getGoogleCat['id_category'].'" value="'.$getGoogleCat['name'].'" title="'.$getGoogleCat['name'].'" placeholder="'.$this->l('click to show all options, type to filter').'" autocomplete="off" />';
        else {  
    ...
    public function renderCategoryList() {
        ...
        $selected_supplier = Configuration::get('LOT_EXPORTERGOOGLE_SELECTED_SUPPLIER');
        
        $suppliername = Db::getInstance()->getValue("SELECT name FROM "._DB_PREFIX_."supplier WHERE id_supplier='".pSQL($selected_supplier)."'");
        
        $coresp_cat_id = (int)Db::getInstance()->getValue("SELECT id_category FROM "._DB_PREFIX_."category_lang WHERE name='".pSQL($suppliername)."' AND id_lang=2");
    
        if ($coresp_cat_id == 0) $coresp_cat_id = -1;
        ...
        */
        ...
        if ($this->getOrderSintax($helperList->list_id) != '') {
            ...
            $count = Db::getInstance()->getValue("SELECT COUNT(distinct t.id_category) as totalitems ". "FROM " . _DB_PREFIX_ .$helperList->table." t INNER JOIN "._DB_PREFIX_.$helperList->table_lang." tl ON tl.id_category=t.id_category WHERE t.id_parent='".pSQL($coresp_cat_id)."' AND tl.id_lang='".pSQL($this->context->language->id)."'".$this->getOrderSintax($helperList->list_id));
            
        }
        
        else {
            
            $count = Db::getInstance()->getValue("SELECT COUNT(distinct t.id_category) as totalitems ". "FROM " . _DB_PREFIX_ .$helperList->table." t INNER JOIN "._DB_PREFIX_.$helperList->table_lang." tl ON tl.id_category=t.id_category WHERE t.id_parent='".pSQL($coresp_cat_id)."' AND tl.id_lang='".pSQL($this->context->language->id)."'");
        ...
        if ($this->getOrderSintax($helperList->list_id) != '') {
            ...
            $helperList->_select = Db::getInstance()->ExecuteS("SELECT t.id_category, t.id_parent, t.level_depth, tl.name, t.id_category AS google_category, t.id_category AS action_id ". "FROM " . _DB_PREFIX_ .$helperList->table." t LEFT JOIN "._DB_PREFIX_.$helperList->table_lang." tl ON tl.id_category=t.id_category WHERE t.id_parent='".pSQL($coresp_cat_id)."' AND tl.id_lang='".pSQL($this->context->language->id)."'".$this->getOrderSintax($helperList->list_id)." LIMIT " . (pSQL($currentpage - 1) * $prodsperpage) . ',' . pSQL($prodsperpage));
            ...
            $helperList->_select = Db::getInstance()->ExecuteS("SELECT t.id_category, t.id_parent, t.level_depth, tl.name, t.id_category AS google_category, t.id_category AS action_id ". "FROM " . _DB_PREFIX_ .$helperList->table." t LEFT JOIN "._DB_PREFIX_.$helperList->table_lang." tl ON tl.id_category=t.id_category WHERE t.id_parent='".pSQL($coresp_cat_id)."' AND tl.id_lang='".pSQL($this->context->language->id)."' LIMIT " . (pSQL($currentpage - 1) * $prodsperpage) . ',' . pSQL($prodsperpage));
            
           
        }
        
        //var_dump($helperList->_select);
    ...
    public function showlist($helperList) {
        ...
        $fields_list = array(
			'id_category' => array('title' => $this->l('ID Category'), 'orderby' => false, 'search' => false, 'remove_onclick' => true,  'class' => 'col-lg-2 nopointer'),
			...
			'name' => array('title' => $this->l('Feed category'), 'orderby' => false, 'search' => false, 'remove_onclick' => true, 'class' => 'nopointer'),
            'google_category' => array('title' => $this->l('Shop Category'), 'orderby' => false, 'search' => false, 'remove_onclick' => true, 'callback' => 'getGoogleCategory'),
    ...
    protected function getConfigForm()
    {
        ...
        $modelNoOptions = array();
            $modelNoOptions[] = array('id_option' => 0, 'name'=> '---');
            $suppliers =  Db::getInstance()->ExecuteS("SELECT * FROM "._DB_PREFIX_."supplier WHERE active=1");
                foreach ($suppliers as $s) {
                $modelNoOptions[] = array('id_option' => $s['id_supplier'], 'name'=> $s['name']);
                }
        $modelNoOptions[] = array('id_option' => 10, 'name'=>'test');
        ...
        return array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                    'type' => 'select' ,
                    'name' => 'LOT_EXPORTERGOOGLE_SELECTED_SUPPLIER',
                    'id' => 'selected_supplier',    
                    'label' => $this->l('Select feed/supplier to map'),
                    'options' => array(
                    'query' => $modelNoOptions , 
                    'id' => 'id_option' ,
                    'name' => 'name' , 
                    )
                    )
    ...
    protected function getConfigFormValues()
    {
        ...
        if (Configuration::get('LOT_EXPORTERGOOGLE_MAIN_GCID')) {
            ...
            //$google_category_main = Db::getInstance()->getValue("SELECT * FROM "._DB_PREFIX_."supplier WHERE active=1");
            
            return array(
            'google_id' => Configuration::get('LOT_EXPORTERGOOGLE_MAIN_GCID'),
            'google_category_main' => '',
            'LOT_EXPORTERGOOGLE_SELECTED_SUPPLIER' => Configuration::get('LOT_EXPORTERGOOGLE_SELECTED_SUPPLIER')    
            );
    ...
    protected function processForm() {
        ...
        Configuration::updateValue('LOT_EXPORTERGOOGLE_SELECTED_SUPPLIER',Tools::getValue('LOT_EXPORTERGOOGLE_SELECTED_SUPPLIER'));
