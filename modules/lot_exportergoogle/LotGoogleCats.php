...
class LotGoogleCats extends Module {
    ...
    public function getCatList($input) {
        ...
        */
        ...
        if ($input['inputval'] != '')
        $cats = Db::getInstance()->ExecuteS("SELECT cl.id_category AS value, cl.name AS text FROM "._DB_PREFIX_."category_lang cl INNER JOIN "._DB_PREFIX_."category c ON cl.id_category=c.id_category WHERE c.id_parent=3 AND cl.name LIKE '%".pSQL($input['inputval'])."%' AND cl.id_lang=2");
        else
        $cats = Db::getInstance()->ExecuteS("SELECT cl.id_category AS value, cl.name AS text FROM "._DB_PREFIX_."category_lang cl INNER JOIN "._DB_PREFIX_."category c ON cl.id_category=c.id_category WHERE c.id_parent=3 AND cl.id_lang=2");
