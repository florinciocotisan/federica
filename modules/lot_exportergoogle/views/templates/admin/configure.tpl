...
<div class="row moduleconfig-content">
    ...
    <input style="display: none;" type="checkbox" {if !empty($restrictions)}{if $restrictions.only_active == 1}checked="checked"{/if}{else}checked="checked"{/if} id="onlyactive" value="1" /><!--<label>{l s='Only active products' mod='lot_exportergoogle'}</label>-->
    <div class="clear"></div>	
<input style="display: none;" type="checkbox" id="combsep" value="0" {if !empty($restrictions)}{if $restrictions.combsep == 1}checked="checked"{/if}{/if} /><!--<label>{l s='Consider product combination as separate product' mod='lot_exportergoogle'}	</label>-->        		
			<div class="clear"></div>	
<p class="info">{l s='If no category, feed or manufacturer is selected, all products will be included in the feed.' mod='lot_exportergoogle'}</p>
	<div class="clear"></div>			
