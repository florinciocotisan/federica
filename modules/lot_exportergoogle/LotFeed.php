...
public function getproductpath($id_product) {
  ...
  $matchingcat = Db::getInstance()->getValue("SELECT google_id FROM lot_exportergoogle_category_map WHERE id_category='".pSQL($smallestcategory[0]['id_category'])."'");
    
  if ($matchingcat) $smallestcategory = $matchingcat;   
  else $smallestcategory = $smallestcategory[0]['id_category'];    
   
  if (Tools::substr(_PS_VERSION_,0,3) == '1.6')
  return strip_tags(Tools::getPath($smallestcategory));
  else
  return $this->getCategoryBreadcrumb($smallestcategory);
...
public function generateCodeXML($id_product, $comb=false, $id_lang, $imageformat = null, $context = null) {
    ...
    $variables['stock'] = 'in stock';
     ...
     $variables['stock'] = $proddata['quantity'];
    ...
    $variables['sale_price'] = number_format(Product::getPriceStatic($id_product, true, $comb, 4, NULL, false, true, 1, false, null, null, null, $specific_price_output, true, false, $context, false),2,'.','').' '.$context->currency->iso_code;    
    }
    else {    
